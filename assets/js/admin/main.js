(function($) {
    ...
    $("body").on("click", "#saveInfoBtn", function(e) {
        ...
        $.ajax({
            ...
            data: {
                ...
                'waiting_time': $('#waiting_time').val(), // waiting time
                'addthis_code': $('#addthis_code').val(), // addthis code 
                'disqus_url': $('#disqus_url').val(), // disqus url
                'color_1': $('#color_1').val(), // color_1
                'color_2': $('#color_2').val(), // color_2
                'color_3': $('#color_3').val(), // color_3
            },
            dataType: 'json',
            success: function(data) {
    ...
    $("body").on("click", "#saveAdsBtn", function(e) {
        ...
        $.ajax({
            ...
            data: {
                ...
                'home_2': $('#home_2').val(),
                'mobile': $('#mobile').val(),
                'download_1': $('#download_1').val(),
                'download_2': $('#download_2').val(),
                'download_3': $('#download_3').val(),
                'blog_1': $('#blog_1').val(),
                'blog_2': $('#blog_2').val(),
                'blog_3': $('#blog_3').val(),
            },
    ...
    $("#icon_path").change(function() {
        var fileExtension = ['png']; // Allowed types
        if ($.inArray($(this).val().split('.').pop().toLowerCase(), fileExtension) == -1) {
            swal("Error!", "Only png are allowed", {
                icon: "error",
                buttons: {
                    confirm: {
                        className: 'btn btn-danger'
                ...
                readFaviconURL = function(input_favicon) {
                    if (input_favicon.files && input_favicon.files[0]) {
                        ...
                        reader.onload = function(e) {
                            ...
                            $('#icon_preview').attr('src', e.target.result); // Preview image
                        }
                        reader.readAsDataURL(input_favicon.files[0]);
                    }
                }
    ...
    $("#image").change(function() {
        var fileExtension = ['png', 'jpg', 'jpeg']; // Allowed types
        if ($.inArray($(this).val().split('.').pop().toLowerCase(), fileExtension) == -1) {
            swal("Error!", "Only png, jpg, jpeg are allowed", {
                icon: "error",
                buttons: {
            ...
            if (favicon) {
                readFaviconURL = function(input_favicon) {
                    if (input_favicon.files && input_favicon.files[0]) {
                        var reader = new FileReader();
                        reader.onload = function(e) {
                            $('.imagebox').removeClass('d-none'); // show box image
                            $('#preview_image').attr('src', e.target.result); // Preview image
                        }
                        reader.readAsDataURL(input_favicon.files[0]);
                    }
                }
            ...
            if (favicon) {
                readFaviconURL = function(input_favicon) {
                    if (input_favicon.files && input_favicon.files[0]) {
                        var reader = new FileReader();
                        reader.onload = function(e) {
                            $('.preview_box').removeClass('d-none'); // show box image
                            $('#feature_image').attr('src', e.target.result); // Preview image
                        }
                        reader.readAsDataURL(input_favicon.files[0]);
                    }
                }
            ...
            if (favicon) {
                readFaviconURL = function(input_favicon) {
                    if (input_favicon.files && input_favicon.files[0]) {
                        var reader = new FileReader();
                        reader.onload = function(e) {
                            $('#hero_image').attr('src', e.target.result); // Preview image
                        }
                        reader.readAsDataURL(input_favicon.files[0]);
                    }
                }
